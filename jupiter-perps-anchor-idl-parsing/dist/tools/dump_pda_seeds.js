// Print PDA seed specs for key accounts from the local IDL.
// Run:
//   TS_NODE_PROJECT=jupiter-perps-anchor-idl-parsing/tsconfig.json \
//     node --loader ts-node/esm \
//     jupiter-perps-anchor-idl-parsing/src/tools/dump_pda_seeds.ts
import { IDL } from "../idl/jupiter-perpetuals-idl.js";
function find(name) {
    const acc = IDL.accounts?.find((a) => a?.name?.toLowerCase?.() === name.toLowerCase());
    if (!acc)
        return null;
    return {
        name: acc.name,
        pda: acc.pda ?? null,
        // show instruction account metas that reference this, for sanity
        usedBy: IDL.instructions
            ?.filter((ix) => ix?.accounts?.some((m) => m?.name === name))
            ?.map((ix) => ix.name),
    };
}
function show(name) {
    const x = find(name);
    console.log("\n=== ", name, " ===");
    if (!x) {
        console.log("(not found in IDL.accounts)");
    }
    else {
        console.log(JSON.stringify(x, null, 2));
    }
}
console.log("IDL has", IDL.accounts?.length ?? 0, "accounts");
show("position");
show("positionRequest");
show("pool");
show("custody");
//# sourceMappingURL=dump_pda_seeds.js.map