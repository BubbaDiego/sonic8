...
function tryCombo(
  ...
  parts: PK[],
  expect?: PK,
): { addr: PK; bump: number; match: boolean } {
...
(async () => {
  ...
  const programId = new PublicKey((PERPS_IDL as any).metadata.address);
  const provider = new AnchorProvider(conn, {} as any, {});
  const program = new Program(PERPS_IDL as Idl, programId, provider);

  // Load singleton pool + custodies (so we have stable inputs)
  const pools = await (program.account as any).pool.all();
  if (!pools?.length) throw new Error("No pool found");
  const poolPk = pools[0].publicKey;
  const poolAcc = pools[0].account;
  const custodies: PK[] = (poolAcc.custodies as PK[]) || [];
  ...
  const parts: PK[] = [owner, poolPk, ...custodies];

  function uniqPKs(arr: PK[]) {
