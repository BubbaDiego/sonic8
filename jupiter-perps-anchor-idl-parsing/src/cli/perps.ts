...
type IncreaseInput = {
  rpcUrl?: string;
  programId?: string;
  idlPath?: string;                 // optional override
  owner: string;                    // required
  fundingAccount: string;           // required (payer ATA or wallet)
  perpetuals: string;               // program state PDA / account in IDL
  pool: string;                     // pool account
  // If you already have them, pass these; otherwise we try to derive "eventAuthority"
  position?: string;
  positionRequest?: string;
  positionRequestAta?: string;

  custody: string;                  // base (SOL) custody
  collateralCustody: string;        // quote (USDC) custody
  inputMint: string;                // EPjF... (USDC)
  referral?: string | null;

  params: {
    sizeUsdDelta: number;           // integer units expected by program
    collateralTokenDelta: number;   // integer units expected by program
    side: "long" | "short" | "Long" | "Short";
    priceSlippage?: number;
    jupiterMinimumOut?: string | null;
    counter?: number;               // u32-ish
  };
};
...
function pkey(s: string) { return new PublicKey(s); }

function eventAuthorityFor(programId: PublicKey) {
...
function ensureOrientation(i: IncreaseInput) {
...
async function simulateIx(conn: Connection, ix: anchor.web3.TransactionInstruction) {
...
function parseAnchorFromLogs(logs?: string[]): {errorCode?:string;account?:string;left?:string;right?:string} {
  if (!logs) return {};
  const out: any = {};
  for (const l of logs) {
    ...
    if (code) out.errorCode = code[1];
    if (l.trim() === "Program log: Left:")  out._leftNext  = true;
    else if (l.trim() === "Program log: Right:") out._rightNext = true;
    else if (out._leftNext && l.startsWith("Program log: "))  { out.left  = l.replace("Program log: ","").trim();  out._leftNext=false; }
    else if (out._rightNext && l.startsWith("Program log: ")) { out.right = l.replace("Program log: ","").trim();  out._rightNext=false; }
  }
  delete out._leftNext; delete out._rightNext;
  return out;
}

async function cmdDryRunIncrease() {
  const input = await readStdinJSON<IncreaseInput>();
  ...
  const accountsStrict: Record<string,string> = {
    owner:                 input.owner,
  ...
  const method = (program.methods as any)["createIncreasePositionMarketRequest"];
  if (!method) {
  ...
  const out: BaseOut = {
    ...
    accounts: {
      ordered: ix.keys.map((k: anchor.web3.AccountMeta) => k.pubkey.toBase58()),
...
async function main() {
  try {
    ...
    prettyError(err);
    writeJSON({ ok:false, error: (err as Error).message });
    process.exit(1);
  }
}
