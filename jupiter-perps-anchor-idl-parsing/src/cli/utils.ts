...
export function writeJSON(obj: unknown) {
  process.stdout.write(JSON.stringify(obj, null, 2) + "\n");
}

export function debugBlock(title: string, lines: string[] = []) {
  const pad = (s:string)=> "  " + s;
  console.error(chalk.gray("┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"));
  console.error(chalk.cyanBright(`┃ ${title}`));
  for (const line of lines) console.error(chalk.gray(pad(line)));
  console.error(chalk.gray("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"));
}

export function debugKV(title: string, kv: Record<string, string>) {
  const lines = Object.entries(kv).map(([k,v]) => `${chalk.white(k)}: ${chalk.gray(v)}`);
  debugBlock(title, lines);
}

export function resolveRepoPath(...p: string[]) {
...
export function prettyError(err: unknown, context?: Record<string,string>) {
  const e = err as any;
  const name = e?.name ?? "Error";
  const msg  = e?.message ?? String(err);
  const extra = e?.stack ? e.stack.split("\n").slice(1, 6).map((s: string) => s.trim()) : [];
  const rows = [
...
export function sideToIdlEnum(side: string) {
