...
import { Card, CardHeader, CardContent, Table, TableHead, TableBody, TableRow, TableCell, TextField, Typography } from '@mui/material';
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              {WINDOWS.map(w => <TableCell key={w} align="center">{w}</TableCell>)}
            </TableRow>
          </TableHead>
          <TableBody>
            {ASSETS.map(a => (
                ...
                {WINDOWS.map(w => (
                  <TableCell key={w} align="center">
                    ...
                    <Typography variant="caption"
                      color={Math.abs(live?.[a]?.[w]?.pct_move || 0) >= (norm[a][w]||0)
