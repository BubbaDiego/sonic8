...
async def get_traders(dl: DataLocker = Depends(get_app_locker)):
    return dl.traders.list_traders()

@router.get("/{name}", response_model=Trader)
async def get_trader(name: str, dl: DataLocker = Depends(get_app_locker)):
...
async def create_trader(trader: Trader, dl: DataLocker = Depends(get_app_locker)):
    if dl.traders.create_trader(trader):
        return {"status": "created"}
    raise HTTPException(status_code=400, detail=dl.traders.last_error)

@router.put("/{name}")
async def update_trader(name: str, trader: dict, dl: DataLocker = Depends(get_app_locker)):
    dl.traders.update_trader(name, trader)
    return {"status": "updated"}

@router.delete("/{name}")
async def delete_trader(name: str, dl: DataLocker = Depends(get_app_locker)):
...
async def quick_import_traders(dl: DataLocker = Depends(get_app_locker)):
...
@router.get("/export")
async def export_traders(dl: DataLocker = Depends(get_app_locker)):
