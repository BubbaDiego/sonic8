...
def list_positions(dl: DataLocker = Depends(get_locker)):
    return PositionCore(dl).get_all_positions()

@router.post("/", status_code=201)
def create_position(pos: PositionDB, dl: DataLocker = Depends(get_locker)):
...
def delete_position(pos_id: str, dl: DataLocker = Depends(get_locker)):
    PositionCore(dl).delete_position(pos_id)
    return {"status": "deleted"}

@router.post("/snapshot")
def snapshot(dl: DataLocker = Depends(get_locker)):
...
async def update_positions(bg: BackgroundTasks, dl: DataLocker = Depends(get_locker)):
...
async def enrich(bg: BackgroundTasks, dl: DataLocker = Depends(get_locker)):
