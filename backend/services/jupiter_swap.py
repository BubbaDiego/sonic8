...
# ---- resilient endpoints (Jupiter changes paths over time) ----
_QUOTE_PATHS: List[str] = [
    "/swap/v1/quote",
    "/v6/quote",
    "/quote/v6",
]
_SWAP_TX_PATHS: List[str] = [
    "/swap/v1/swap",
    "/swap/v1/transaction",
    "/swap/v6/transaction",
    "/v6/swap",
...
def get_quote(
    *,
    ...
    """
    If restrict_intermediates is False, we OMIT the param entirely to let Jupiter pick any route.
    Sending 'restrictIntermediateTokens=false' to lite-api can behave badly / 404. Omit instead.
    """
    params = {
...
# ---------------- Prices / portfolio / safe-max ----------------
def _fetch_prices_usdc(ids: List[str]) -> Dict[str, float]:
    prices: Dict[str, float] = {}
    try:
        if ids:
            r = requests.get("https://price.jup.ag/v6/price", params={"ids": ",".join(ids), "vsToken": "USDC"}, timeout=8)
            r.raise_for_status()
            data = r.json().get("data", {})
            for k, v in data.items():
                try: prices[k] = float(v.get("price"))
                except Exception: pass
            if all(k in prices for k in ids): return prices
    except Exception: pass
    # Fallback: 1 token -> USDC
    for mint in ids:
        try:
            dec = next((t["decimals"] for t in TOKENS.values() if t["mint"] == mint), 9)
            q = get_quote(input_mint=mint, output_mint=TOKENS["USDC"]["mint"], amount=10**dec,
...
def _sum_token_amount_for_owner(cl: SolClient, owner: Pubkey, mint: Pubkey) -> Tuple[int, int]:
    ...
    accounts = []
    try: accounts = [acc.pubkey for acc in resp.value]
    except Exception: accounts = [a["pubkey"] for a in resp["result"]["value"]]
    ...
    for pk in accounts:
        ...
        try: val = bal.value; amt_raw = int(val.amount); dec = int(val.decimals)
        except Exception: val = bal["result"]["value"]; amt_raw = int(val["amount"]); dec = int(val["decimals"])
        total_raw += amt_raw; decimals = dec
    if decimals is None: decimals = 9
...
def estimate_sol_spend_lamports(owner_pubkey: str, out_mint: str) -> Dict[str, Any]:
    cl = SolClient(RPC_URL); owner = Pubkey.from_string(owner_pubkey)
...
def get_portfolio(owner_pubkey: str, mints: Optional[List[str]] = None) -> Dict[str, Any]:
    cl = SolClient(RPC_URL); owner = Pubkey.from_string(owner_pubkey)
    if not mints: mints = [TOKENS[k]["mint"] for k in ("SOL", "USDC", "mSOL", "JitoSOL")]
...
def project_round_trip(owner_pubkey: str, in_mint: str, out_mint: str,
    ...
    return {"a2b": {"outAmount": int(q1.get("outAmount", 0)), "minOut": b_min_out,
                    "priceImpactPct": float(q1.get("priceImpactPct", 0.0))},
            "b2a": {"minOutA": a_back_min_out},
...
def _parse_actual_b_received_atoms(cl: SolClient, sig: str, owner: Pubkey, out_mint: str) -> Optional[int]:
    try:
        ...
        try:
            ...
            return None
    try: pre = meta["preTokenBalances"]; post = meta["postTokenBalances"]
    ...
    def _sum(listing):
        ...
        for e in listing:
            try:
                if e.get("mint") == out_mint and e.get("owner"):
                    if e.get("owner") == str(owner):
...
def log_swap(owner_pubkey: str, sig: str, in_mint: str, out_mint: str,
    ...
    price_in = float(prices.get(in_mint, 0.0)); price_out = float(prices.get(out_mint, 0.0))
    ...
    actual_edge = None
    if b_recv_atoms is not None and price_in:
        try:
            ...
            actual_edge = {"atoms": int(edge_atoms), "bps": float(edge_bps),
                           "usd": (edge_atoms / (10 ** in_dec)) * price_in}
        except Exception:
            actual_edge = None
    ...
    entry = {
        ...
        "actual": {"pnlUsd": pnl_usd, "valueInUsd": value_in_usd,
                   "valueOutUsd": value_out_usd, "feesUsd": fees_usd,
                   "edge": actual_edge, "prices": prices},
