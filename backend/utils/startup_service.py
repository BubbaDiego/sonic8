...
class StartUpService:
    ...
    def run_all(play_sound: bool = True):
        ...
        try:
            ...
            if play_sound:
                ...
                log.info("🔈 Startup death sound suppressed", source="StartUpService")
            raise
    ...
    def check_for_mother_brain():
        maybe_create_mother_brain(MOTHER_DB_PATH)
    ...
    def ensure_alert_thresholds():
        if not os.path.exists(ALERT_THRESHOLDS_PATH):
            ...
            # Validate the newly created default file to ensure it meets the
            # expected schema. If validation fails, abort startup early so the
            # user can fix the configuration before proceeding.
            ...
            if not valid:
                raise SystemExit(
                    "Startup check failed: default alert_thresholds.json schema invalid"
                )
        else:
    ...
    def verify_required_tables():
        ...
        code = verify_all_tables_exist(str(MOTHER_DB_PATH))
        if code != 0:
