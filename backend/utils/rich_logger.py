...
try:
    ...
    class RichHandler(logging.StreamHandler):
        """Minimal stand-in if Rich is unavailable.

        This handler supports very basic markup such as ``[blue]`` or
        ``[bold red]`` so log output still has some color when the ``rich``
        ...
        def __init__(self, *args, markup: bool = False, **kwargs):
            super().__init__()
            self.markup = markup
            self.level_styles = {}
        ...
        def emit(self, record: logging.LogRecord) -> None:  # type: ignore[override]
            ...
            if self.markup:
                msg = self._apply_markup(msg)
            self.stream.write(msg + self.terminator)
            self.flush()

        def _apply_markup(self, text: str) -> str:
            """Replace ``[color]`` markup with ANSI sequences."""

            def repl(match: re.Match) -> str:
                token = match.group(1).strip()
                if token.startswith("/"):
                    return "\033[0m"
                codes = [self.ANSI_CODES.get(t) for t in token.split()]
