...
def fuzzy_match_key(
    ...
    for candidate in candidate_list:
        # Use a hybrid scoring approach to avoid false positives when the input
        # contains only a very small substring of the candidate.  ``hybrid_score``
        # combines partial and token set ratios on the normalized input without
        # aggressively scrubbing characters which previously caused random
        # strings like "randomjunk" to match ``"avg_leverage"`` with a perfect
        # score when ``rapidfuzz`` was installed.
        score = hybrid_score(norm_input, candidate)
