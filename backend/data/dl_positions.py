...
class DLPositionManager:
    ...
    def create_position(self, position: dict):
        ...
        try:
            ...
            if cursor is None:
                log.error("DB unavailable for position insert", source="DLPositionManager")
                return
    ...
    def get_all_positions(self) -> list:
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while fetching positions", source="DLPositionManager")
                return []
            try:
    ...
    def delete_position(self, position_id: str):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable, cannot delete position", source="DLPositionManager")
                return
    ...
    def delete_all_positions(self):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable, cannot wipe positions", source="DLPositionManager")
                return
    ...
    def get_active_positions(self) -> list:
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while fetching active positions", source="DLPositionManager")
                return []
    ...
    def get_active_positions_by_wallet(self, wallet_name: str) -> list:
        ...
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while fetching active positions by wallet", source="DLPositionManager")
                return []
            cursor.execute(
    ...
    def get_position_by_id(self, pos_id: str):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while fetching position", source="DLPositionManager")
                return None
