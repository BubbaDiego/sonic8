...
class DLModifierManager:
    ...
    def ensure_table(self):
        ...
        if cursor is None:
            log.error("DB unavailable, modifiers table not ensured", source="DLModifierManager")
            return
        cursor.execute("""
    ...
    def set_modifier(self, key: str, value: float, group: str = "heat_modifiers"):
        cursor = self.db.get_cursor()
        if cursor is None:
            log.error("DB unavailable, cannot set modifier", source="DLModifierManager")
            return
        cursor.execute("""
    ...
    def get_modifier(self, key: str) -> float:
        cursor = self.db.get_cursor()
        if cursor is None:
            log.error("DB unavailable, cannot read modifier", source="DLModifierManager")
            return None
    ...
    def get_all_modifiers(self, group: str = None) -> dict:
        cursor = self.db.get_cursor()
        if cursor is None:
            log.error("DB unavailable, cannot fetch modifiers", source="DLModifierManager")
            return {}
        if group:
    ...
    def export_to_json(self) -> str:
        cursor = self.db.get_cursor()
        if cursor is None:
            log.error("DB unavailable, cannot export modifiers", source="DLModifierManager")
            return "{}"
