...
class DLSystemDataManager:
    ...
    def get_theme_mode(self) -> str:
        try:
            ...
            if cursor is None:
                log.error("DB unavailable while fetching theme mode", source="DLSystemDataManager")
                return "light"
    ...
    def set_theme_mode(self, mode: str):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable, cannot set theme mode", source="DLSystemDataManager")
                return
    ...
    def get_last_update_times(self) -> SystemVariables:
        cursor = self.db.get_cursor()
        if cursor is None:
            log.error("DB unavailable while fetching update times", source="DLSystemDataManager")
    ...
    def set_last_update_times(self, updates: dict):
        ...
        cursor = self.db.get_cursor()
        if cursor is None:
            log.error("DB unavailable while setting update times", source="DLSystemDataManager")
            return
        cursor.execute("""
            UPDATE system_vars
    ...
    def insert_or_update_theme_profile(self, name: str, config: dict):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while saving theme profile", source="DLSystemDataManager")
                return
            cursor.execute("""
    ...
    def get_theme_profiles(self) -> dict:
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while fetching theme profiles", source="DLSystemDataManager")
                return {}
    ...
    def delete_theme_profile(self, name: str):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable, cannot delete theme profile", source="DLSystemDataManager")
                return
    ...
    def set_active_theme_profile(self, name: str):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable, cannot set active theme profile", source="DLSystemDataManager")
                return
    ...
    def get_active_theme_profile(self) -> dict:
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while retrieving active profile", source="DLSystemDataManager")
                return {}
    ...
    def get_var(self, key: str):
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while reading system var", source="DLSystemDataManager")
                return None
    ...
    def set_var(self, key: str, value: dict):
        ...
        try:
            cursor = self.db.get_cursor()
            if cursor is None:
                log.error("DB unavailable while writing system var", source="DLSystemDataManager")
                return
            cursor.execute("""
