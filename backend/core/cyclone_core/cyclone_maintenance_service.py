...
class CycloneMaintenanceService:
    ...
    def clear_all_tables(self):
        ...
        # Ensure profit badge does not linger after data wipe
        try:
            self.dl.system.set_var("profit_badge_value", None)
        except Exception as e:  # pragma: no cover - unlikely failure
            log.error(f"❌ Failed to reset profit badge: {e}", source="SystemMaintenanceService")
