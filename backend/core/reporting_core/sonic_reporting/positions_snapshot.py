...
def render(dl, csum: Dict[str, Any]) -> None:
    """
    Render the Positions Snapshot with **no title row** and **no extra spacers**.
    The table starts immediately with the column headers.
    """
    ...
    # No title row → pass None; writer will render only headers + rows with no leading blank line.
    write_table(None, headers, rows if rows else [["—","—","—","—","—","—","—"]])
