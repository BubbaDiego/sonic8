...
# ===== theming: standard title + body knobs =====
from .theming import (
    emit_title_block,
...
# ===== colors/icons from legacy view (kept) =====
...
# ----- emoji-safe display width helpers (kept from legacy) -----
...
# ===== metric/threshold/state formatting (kept) =====
...
# ===== data loading (legacy logic preserved; manager-first, table/ledger tolerant) =====
...
# ===== console_panels render (returns list[str]) =====
def render(context: Dict[str,Any], width: Optional[int] = None) -> List[str]:
    # Accept either context dict (preferred) or dl directly (legacy) for safety
    dl = context.get("dl") if isinstance(context, dict) else context
    ...
    # Rounded title, zero title padding/border rails handled by emit_title_block
    out += emit_title_block(PANEL_SLUG, PANEL_NAME)

    # Column header — no underline between header and values
    header = (
        INDENT
        + _pad("", W_ICON)
        + _pad("Mon",     W_MON)
    ...
    if not rows:
        out += body_indent_lines(PANEL_SLUG, ["(no monitor results)"])
