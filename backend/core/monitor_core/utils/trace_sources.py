...
# --- Lightweight JSON reader (no config_loader import) ---
_ENV_JSON_PATH = "SONIC_CONFIG_JSON"
_JSON_CANDIDATES = ("backend/config/sonic_monitor_config.json", "config/sonic_monitor_config.json")
    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            if str(k).lower() == target.lower():
                return v
            got = _find_key_recursive(v, target)
            if got is not None:
                return got
...
def read_monitor_threshold_sources(dl) -> tuple[dict, str]:
    ...
    profit_pos = pick(("profit_position_threshold", "profit_threshold", "profit_badge_value")); mark(profit_pos.get("source"))
    profit_pf  = pick(("profit_portfolio_threshold", "profit_total_threshold", "profit_total")); mark(profit_pf.get("source"))
    liq_btc    = pick(("liquid_threshold_btc", "liquid_threshold_BTC", "liquid_threshold"));   mark(liq_btc.get("source"))
    liq_eth    = pick(("liquid_threshold_eth", "liquid_threshold_ETH", "liquid_threshold"));   mark(liq_eth.get("source"))
    liq_sol    = pick(("liquid_threshold_sol", "liquid_threshold_SOL", "liquid_threshold"));   mark(liq_sol.get("source"))
...
def pretty_print_trace(trace: Dict[str, List[Tuple[str, str, Any, bool]]]) -> None:
    def _fmt(v: Any) -> str: return "—" if v in (None, "") else str(v)
