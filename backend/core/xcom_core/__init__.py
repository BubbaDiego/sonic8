"""Consolidated dispatcher for :mod:`backend.core.xcom_core`.

This version routes directly through the same guard + provider stack that
monitors use: XComConfigService + xcom_ready + VoiceService.
It keeps the public API name `dispatch_notifications` and the channel
normalization helpers for compatibility with console/LaunchPad callers.
"""
...
# Consolidated stack (same as monitors use)
...
# We still export XComCore for backward import compatibility, but the
# consolidated dispatcher below no longer calls it.
try:  # pragma: no cover
    from .xcom_core import XComCore  # type: ignore
...
def _as_channel_map(
    ...
    if isinstance(channels, Mapping):
        explicit = {k: bool(v) for k, v in channels.items()}
    ...
    # If caller gave nothing, fall back to JSON-configured channels for monitor
...
def dispatch_notifications(
    *,
    ...
    """
    Consolidated dispatcher used by console/tests/monitors.

    Behavior:
      • Channel selection comes from caller or JSON defaults (XComConfigService).
      • Voice path fires only when `result['breach'] is True` AND `xcom_ready(...)`.
      • Blast radius is display-only (never gates dispatch).
      • Twilio creds resolve Twilio-first (env/context), same as VoiceService.
    ...
    if chan.get("voice", False):
        ...
        if not breach:
            ...
            try:
                # Prefer "api" provider; fallback to "twilio" (both patterns exist in repo)
                provider_cfg = cfg.get_provider("api") or cfg.get_provider("twilio") or {}
                if provider_cfg is not None and provider_cfg.get("enabled", True) is False:
                ...
                summary["channels"]["voice"] = {"ok": False, "skip": "twilio-error", "error": str(exc)[:160]}
    else:
