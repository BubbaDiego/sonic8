...
## 0. TL;DR for GPTs
- **Purpose:** Sonic is a Python/React/FastAPI system for [YOUR DOMAIN].
- **Primary entrypoints:**
...
See `docs/spec/codebase_map.md` (generated by `make spec-sync`)

## 4. APIs
- OpenAPI: `api/openapi.yaml` (generated; authoritative endpoints)
- Auth scheme: [JWT/Bearer/API Key — fill in]
...
- `make openapi` → refresh API spec
- `make spec-sync` → refresh code map (and optional schemas)
...
## 12. Changelog (Spec)
- 2025-08-16: Initial Spec Bundle added.
