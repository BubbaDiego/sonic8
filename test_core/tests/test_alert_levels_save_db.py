...
MOTHER_DB_PATH = "C:/v0.8/data/mother_brain.db"  # 🔧 update path if needed

def fetch_alert_levels():
    if not os.path.exists(MOTHER_DB_PATH):
        print(f"❌ DB not found at: {MOTHER_DB_PATH}")
        return

    conn = sqlite3.connect(MOTHER_DB_PATH)
