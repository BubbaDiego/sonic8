...
class OperationsMonitor(BaseMonitor):
    ...
    def __init__(self, timer_config_path=None, ledger_filename=None, monitor_interval=300, continuous_mode=False, notifications_enabled=False):
        ...
        self.data_locker = DataLocker(str(MOTHER_DB_PATH))
    ...
    def run_startup_post(self) -> dict:
        ...
        test_pattern = Path(__file__).resolve().parents[1] / "test_core" / "tests" / "test_*.py"

        files = sorted(glob.glob(str(test_pattern)))
        if not files:
            log.warning(f"Skipping POST tests, pattern not found: {test_pattern}", source=self.name)
    ...
    def run_configuration_test(self) -> dict:
        ...
        if not file_exists:
            ...
            try:
                ...
                log.error(
                    f"Alert thresholds configuration invalid ({config_path}): {exc}",
                    source=self.name,
                )
